...
class dboperations
{
	...
	{
		$stmt = $this->conn->prepare("SELECT * FROM requests WHERE req_from_user_phone = ? ORDER BY req_create_datetime");
	...
	{
		$stmt = $this->conn->prepare("SELECT * FROM requests WHERE req_to_user_phone = ? ORDER BY req_create_datetime");
